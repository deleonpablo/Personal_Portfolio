...
const Armenia = ({isRotating, setCurrentStage, setIsRotating, currentStage, ...props }) => {
...
useFrame(() => {
  if (!isRotating) {
    ...
    // ✅ Don't touch setCurrentStage here — hold whatever stage was last set
    return;
  }

  // ✅ Only update stage while actively dragging
  const rotation = islandRef.current.rotation.y;
  const normalizedRotation = ((rotation % (2 * Math.PI)) + 2 * Math.PI) % (2 * Math.PI);

  switch (true) {
    case normalizedRotation >= 0 && normalizedRotation <= 0.5:
      setCurrentStage(1); break;
